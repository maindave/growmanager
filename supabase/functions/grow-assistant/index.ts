...
- La configuración y telemetría local del Wemos, cuando está accesible, aparece en CONTEXTO.deviceContext. Usala para responder preguntas sobre iluminación, temperatura, relays y ventilación. Si connected=false, aclaralo y distinguí los últimos valores conocidos.
...
Deno.serve(async req=>{
  ...
  try{
    ...
    const query=async(table:string,select:string,limit=50)=>{const{data,error}=await supabase.from(table).select(select).eq('workspace_id',workspaceId).limit(limit);if(error)throw error;return data||[]}
    const[{data:workspace,error:workspaceError},cultivations,spaces,lots,products,recipes,recipeVersions,irrigations,activities,agendaEvents]=await Promise.all([supabase.from('workspaces').select('id,name,description,grow_type,started_on').eq('id',workspaceId).single(),query('cultivations','id,name,start_date,end_date,planned_end_date,cultivation_mode,current_stage,stage_started_on,status,notes'),query('spaces','id,cultivation_id,name,description,operational_stage,active'),query('lots','id,cultivation_id,space_id,name,description,stage,show_on_calendar,timeline_started_on,timeline_end_on,active'),query('products','id,name,brand,type,base_unit,description,notes,active'),query('recipes','id,name,type,description,notes,active,current_version_id'),query('recipe_versions','id,recipe_id,version,target_ph,target_ec,notes,recipe_items(product_id,product_name_snapshot,product_brand_snapshot,amount,unit)'),query('irrigation_events','id,cultivation_id,lot_id,scheduled_at,status,water_liters,ph,ec,notes,supplies'),query('activities','id,cultivation_id,space_id,lot_id,plant_id,type,occurred_at,observations,details'),query('agenda_events','id,title,event_type,starts_at,ends_at,cultivation_id,lot_id,description,priority,status,recurrence,archived_at')])
    if(workspaceError)throw workspaceError
    const suppliedDevice=body?.deviceContext&&typeof body.deviceContext==='object'?body.deviceContext:null,deviceContext=suppliedDevice?{connected:Boolean(suppliedDevice.connected),device:String(suppliedDevice.device||'').slice(0,80),temperature:Number.isFinite(Number(suppliedDevice.temperature))?Number(suppliedDevice.temperature):null,humidity:Number.isFinite(Number(suppliedDevice.humidity))?Number(suppliedDevice.humidity):null,config:suppliedDevice.config||suppliedDevice.lastKnownConfig||null,relays:Array.isArray(suppliedDevice.relays)?suppliedDevice.relays.slice(0,4):[]}:null
    const context={now:new Date().toISOString(),workspace,role:membership.role,deviceContext,cultivations,spaces,lots,products,recipes,recipeVersions,agendaEvents:agendaEvents.filter((item:any)=>!item.archived_at).sort((a:any,b:any)=>String(a.starts_at).localeCompare(String(b.starts_at))).slice(-100),irrigations:irrigations.sort((a:any,b:any)=>String(a.scheduled_at).localeCompare(String(b.scheduled_at))).slice(-30),activities:activities.sort((a:any,b:any)=>String(b.occurred_at).localeCompare(String(a.occurred_at))).slice(0,20)}
    const apiKey=Deno.env.get('GEMINI_API_KEY')
    if(!apiKey)return json({error:'gemini_not_configured'},503)
    const history=Array.isArray(body.history)?body.history.slice(-4):[]
    const contents=[...history.map((item:any)=>({role:item.role==='assistant'?'model':'user',parts:[{text:String(item.text||'').slice(0,1600)}]})),{role:'user',parts:[{text:`CONSULTA:\n${message}\n\nCONTEXTO VIVO DEL PROYECTO:\n${JSON.stringify(context).slice(0,30000)}`}]}]
    const request=()=>fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent',{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':apiKey},body:JSON.stringify({system_instruction:{parts:[{text:systemInstruction+' Devolvé el payload como JSON serializado en proposal.payloadJson.'}]},contents,generationConfig:{temperature:0.2,responseMimeType:'application/json',responseSchema:{type:'OBJECT',required:['reply','proposal'],properties:{reply:{type:'STRING'},proposal:{type:'OBJECT',required:['type','summary','payloadJson'],properties:{type:{type:'STRING',enum:['none','schedule_irrigation','postpone_irrigation','create_event','update_event','plan_cultivation','record_activity','record_incident']},summary:{type:'STRING'},payloadJson:{type:'STRING'}}}}}}})});let response=await request();if([429,500,502,503,504].includes(response.status)){await new Promise(resolve=>setTimeout(resolve,700));response=await request()}
