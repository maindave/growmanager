(() => {
  ...
  function validateCultivation(input={}){const errors=[];const value={...input,name:normalizeText(input.name),status:input.status||'active',cultivationMode:input.cultivationMode||'cycle',currentStage:input.currentStage||'vegetative',stageStartedOn:input.stageStartedOn||input.startDate,plannedEndDate:input.plannedEndDate||null};if(!value.name)errors.push('El nombre del cultivo es obligatorio.');if(!isEnumValue(value.status,CULTIVATION_STATUSES))errors.push('El estado del cultivo no es válido.');if(!isEnumValue(value.cultivationMode,CULTIVATION_MODES))errors.push('El tipo de seguimiento no es válido.');if(!isEnumValue(value.currentStage,LOT_STAGES))errors.push('La etapa actual no es válida.');if(value.plannedEndDate&&value.plannedEndDate<value.startDate)errors.push('La finalización estimada no puede ser anterior al inicio.');return{valid:errors.length===0,errors,value}}
